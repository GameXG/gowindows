...
\);`)
	...
	for _, s := range pat.FindAllSubmatchIndex(src, -1) {
		...
		res = pat.Expand(res, []byte(`// ${1}
var ${2} = GUID{
	${3},
	${4},
	${5},
	[8]byte{${6}, ${7}, ${8}, ${9}, ${10}, ${11}, ${12}, ${13}},
}

`), src, s)
	}

	pat2 := regexp.MustCompile(`#define ([^ \r\n]+) (?:\\\s*)?([^ \\\r\n]+)`)

	for _, s := range pat2.FindAllSubmatchIndex(src, -1) {
		res = pat.Expand(res, []byte("\r\nvar ${1} = ${2}\r\n"), src, s)
		// fmt.Print(string(res))
	}

	f, err := os.OpenFile("Fwpuclntguid.go", os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0666)
	if err != nil {
		panic(err)
	}
	defer f.Close()
